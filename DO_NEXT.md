...
1. Stand up the SDK generation pipeline for `M1-P0-T4`, deciding on the generator + target package structure so `packages/shared` can emit deterministic clients from `openapi/blerp.v1.yaml`.
2. Share the updated spec for review (internal approval log) once lint/preview tasks are ready, ensuring reviewers can trace new invitations/audit/config endpoints back to DESIGN_DOCUMENT.md.
