...
## 2026-03-20 — Backend SDK Expansion (clerkClient() Parity)

- Summary: Expanded `@blerp/backend` from 5 methods / 2 namespaces to 26 methods / 6 namespaces. Split monolithic `index.ts` into namespace modules under `api/`. Added `BlerpAPIError` structured error class with `throwIfError` helper. New namespaces: `organizationMemberships`, `invitations`, `sessions`, `emailAddresses`. New methods: `createUser`, `updateUser`, `deleteUser`, `restoreUser`, `listOrganizations`, `createOrganization`, `updateOrganization`, `deleteOrganization`, `listMemberships`, `createMembership`, `updateMembership`, `deleteMembership`, `listInvitations`, `createInvitation`, `revokeInvitation`, `listSessions`, `revokeSession`, `listEmailAddresses`, `createEmailAddress`, `deleteEmailAddress`, `setPrimaryEmailAddress`.
- Tests run: `bun test` in packages/backend (28/28 pass), `bun run typecheck` (6/6 pass), `bun test` in apps/api (25/25 pass — 38 pre-existing failures unchanged)
- Files touched: `packages/backend/src/errors.ts` (new), `packages/backend/src/api/users.ts` (new), `packages/backend/src/api/organizations.ts` (new), `packages/backend/src/api/organization-memberships.ts` (new), `packages/backend/src/api/invitations.ts` (new), `packages/backend/src/api/sessions.ts` (new), `packages/backend/src/api/email-addresses.ts` (new), `packages/backend/src/index.ts` (refactored), `packages/backend/src/__tests__/client.test.ts` (new), `packages/backend/tsconfig.json` (exclude tests)

## 2026-03-20 — Monite SDK Demo Setup + E2E Screenshot Tests
