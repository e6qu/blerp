...
*Objective*: implement per-customer SQLite schemas, migrations, and Drizzle data access layers for users, sessions, projects.

Tasks:
1. Define Drizzle schema per `DESIGN_DOCUMENT.md` (users, sessions, projects, API keys, audit logs) targeting SQLite 3.
...
*Objective*: expose REST endpoints for sign-up, sign-in, session refresh/revoke, and token issuance.
...
*Objective*: build SPA flows that hit the REST API directly and validate compatibility using the official Clerk SDKs.
...
*Objective*: establish minimum security/observability posture and developer documentation to unblock later milestones.
